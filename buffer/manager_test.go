...
func TestBufferManager(t *testing.T) {
	...
	t.Run("test buffer manager", func(t *testing.T) {
		...
		bufs[0], err = bm.Pin(file.NewBlock(fileName, 0))
		require.NoError(t, err)
		bufs[1], err = bm.Pin(file.NewBlock(fileName, 1))
		require.NoError(t, err)
		bufs[2], err = bm.Pin(file.NewBlock(fileName, 2))
		require.NoError(t, err)
		err = bm.Unpin(bufs[1])
		require.NoError(t, err)
		bufs[1] = nil
		bufs[3], err = bm.Pin(file.NewBlock(fileName, 0)) // block 0 pinned twice
		require.NoError(t, err)
		bufs[4], err = bm.Pin(file.NewBlock(fileName, 1)) // block 1 repinned
		require.NoError(t, err)
