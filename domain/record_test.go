...
func TestSchema_AddField(t *testing.T) {
	...
	mp1[fld1] = domain.NewFieldInfo(domain.Int32FieldType, 0)
	intSch.SetInfo(mp1)
	...
	mp2[fld2] = domain.NewFieldInfo(domain.StringFieldType, 8)
	strSch.SetInfo(mp2)
	...
	tests := []struct {
	...
	}{
		{
			name:     "add int field",
			fldname:  fld1,
			typ:      domain.Int32FieldType,
			expected: intSch,
		},
		{
			name:     "add int field",
			fldname:  fld2,
			typ:      domain.StringFieldType,
			length:   8,
			expected: strSch,
		},
	}
...
func TestSchema_AddStringField(t *testing.T) {
	...
	mp1 := make(map[domain.FieldName]*domain.FieldInfo)
	mp1[fld1] = domain.NewFieldInfo(domain.StringFieldType, 8)
	strSch.SetInfo(mp1)
...
func TestSchema_Add(t *testing.T) {
	...
	mp1 := make(map[domain.FieldName]*domain.FieldInfo)
	mp1[fld1] = domain.NewFieldInfo(domain.StringFieldType, 8)
	mp1[fld2] = domain.NewFieldInfo(domain.Int32FieldType, 0)
	bsch.SetInfo(mp1)

	strSch := domain.NewSchema()
	mp2 := make(map[domain.FieldName]*domain.FieldInfo)
	mp2[fld1] = domain.NewFieldInfo(domain.StringFieldType, 8)
	strSch.SetFields([]domain.FieldName{fld1})
	strSch.SetInfo(mp2)

	intSch := domain.NewSchema()
	mp3 := make(map[domain.FieldName]*domain.FieldInfo)
	mp3[fld2] = domain.NewFieldInfo(domain.Int32FieldType, 0)
...
func TestLayout(t *testing.T) {
	t.Run("constructs Layout", func(t *testing.T) {
		...
		schema.AddField("hoge", domain.Int32FieldType, 0)
		schema.AddField("piyo", domain.StringFieldType, 8)
